# Script for loading generic clone file 
...
class Generic_Clone(Clone):
    
    def __init__( self, verbose, repository_tag, repository_chunk, airr_map, repository):
        Clone.__init__(self, verbose, repository_tag, repository_chunk, airr_map, repository)
        # The annotation tool used for the parser is unknown
        self.setAnnotationTool("Other")
        # The default column in the AIRR Mapping file is ir_general. This can be 
    ...
    def process(self, filewithpath):
        ...
        # This reads one file at a time, given the full file (path) name
        ...
        if not os.path.isfile(filewithpath):
            print("ERROR: Could not open clone file ", filewithpath)
            return False
        ...
        if filewithpath.endswith(".gz"):
            ...
            with gzip.open(filewithpath, 'rb') as file_handle:
                ...
                success = self.processTSVFile(file_handle, filename)
    ...
    def processTSVFile( self, file_handle, filename ):
        ...
        # We select the rows in the mapping that contain fields of interest.
        # At this point, file_fields contains N columns that contain our mappings for
        # the specific formats (e.g. ireceptor, airr, vquest). 
	...
	# Get a Pandas iterator for the file. When reading the file we only want to
        ...
        for df_chunk in df_reader:
            ...
            for clone_column in df_chunk.columns:
                if clone_column in columnMapping:
                    mongo_column = columnMapping[clone_column]
                    if self.verbose():
                        print("Info: Mapping %s field in file: %s -> %s"
                              %(self.getAnnotationTool(), clone_column, mongo_column))
                    df_chunk.rename({clone_column:mongo_column},
                                    axis='columns', inplace=True)
                else:
                    if self.verbose():
                        print("Info: No mapping for %s column %s, storing as is"
                              %(self.getAnnotationTool(), clone_column))
            # Check to see which desired mappings we don't have...
            ...
            # clone file might not have junction nucleotide length, we want it in our
            # repository.
            ...
            if not locus in df_chunk:
                df_chunk[locus] = df_chunk[v_call].apply(Annotation.getLocus)
